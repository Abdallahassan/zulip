...
function message_hover(message_row) {
    ...
    if ((message_edit.get_editability(message) === message_edit.editability_types.FULL) &&
        ...
        message_row.find(".edit_content")
            .html('<i class="fa fa-pencil edit_content_button" aria-hidden="true" title="Edit"></i>');
    } else {
        message_row.find(".edit_content")
            .html('<i class="fa fa-file-text-o edit_content_button"' +
                'aria-hidden="true" title="View source" data-msgid="' + id + '"></i>');
    }
}
